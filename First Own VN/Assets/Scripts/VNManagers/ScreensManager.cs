...
public class ScreensManager : MonoBehaviour {
    ...
    public float LongPauseTime = 3; //Время длительной паузы
    ...
    public void End(string title, string text) //Вставка концовки
    {
        ScreensObject.SetActive(true); //Делаем родительский объект активным
    ...
    IEnumerator end(string title, string text)
    {
        ...
        StartCoroutine(FadeObject(EndObject, true, FadeTime)); //Показываем фон
        yield return StartCoroutine(WaitNext()); //Ждём
        Text[] txts = EndObject.GetComponentsInChildren<Text>(); //Находим все компоненты Text
        txts[0].text = title; //Ставим заголовок
        txts[1].text = text; //Ставим основной текст
        for (int i = 0; i < txts.Length; i++) //Для каждого такого компонента
        {
            StartCoroutine(FadeObject(txts[i], true, FadeTime)); //Показываем текст
            yield return StartCoroutine(WaitNext()); //Ждём
        }
        float tm = 0; //Счётчик паузы
        while (tm < LongPauseTime) //Пока пауза не закончилась
        {
